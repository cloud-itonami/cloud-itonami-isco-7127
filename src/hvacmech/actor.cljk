(ns hvacmech.actor
  "HvacMechActor — the ISCO-08 7127 AC/refrigeration service
  ...
  housebuilder.actor (closest domain shape, itself modeled on
  ...
  The unconditional invariant: the HVAC Mechanic Advisor can never
  directly commit an operating record the HvacMechGovernor refuses,
  and can never make this actor finalize a
  refrigerant-system-service-execution decision (e.g. proceeding with
  a specific refrigerant-system service) or override/bypass a
  technician-certification requirement — every commit-record! call is
  gated behind `:decide`, and that decision class is a permanent hard
  block regardless of disposition path. This actor coordinates SERVICE
  SCHEDULING/LOGISTICS ONLY — it never performs refrigeration-system
  service or authorizes refrigerant-system service itself."
...
(defn build-graph
  "Build a compiled HvacMechActor graph. `store` implements
  `hvacmech.store/Store`. `advisor` implements
  `hvacmech.advisor/Advisor` (defaults to `mock-advisor`).
